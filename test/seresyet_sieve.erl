...
run_sieve() ->
    Start = now(),
    Engine1 = seresye_engine:assert(seresye_engine:new(), [{X} || X <- lists:seq (2, 100)]),
    Engine2 = seresye_engine:add_rules(Engine1, ?MODULE),
    Engine3 = seresye_engine:assert(Engine2, {is, started}),
    End = now(),
    Expected = lists:sort([{is,finished}, {97}, {89}, {83}, {79}, {73}, {71}, {67}, {61}, {59}, {53}, {47}, {43}, {41},
                           {37}, {31}, {29}, {23}, {19}, {17}, {13}, {11}, {7}, {5}, {3}, {2}]),
    ?assertEqual(Expected, lists:sort(seresye_engine:get_kb (Engine3))),
